...
def start_loop():
    global run_scanner, scanner_thread
    # Stoppe den alten Scanner-Thread, falls er läuft
    # if scanner_thread and scanner_thread.is_alive():
    #     stop_loop()  # Setze run_scanner auf False
    #     scanner_thread.join()  # Warte, bis der alte Thread beendet ist

    run_scanner = True
    # Starte einen neuen Thread
    chestpress = Chestpress()

    scanner_thread = threading.Thread(target=chestpress.exercise)
...
run_scanner = False  # Variable, die die Ausführung der Schleife steuert
scanner_thread = None  # Variable für den Thread
...
class Chestpress():
    ...
    def exercise(self, source=0):
        ...
        while run_scanner:
                ...
                """try:     Erstmal rausnehmen, nicht genügend Punkte 
                    # Schulter - Hüfte -  Knie
                    if 12 in idx_to_coordinates and 24 in idx_to_coordinates and 26 in idx_to_coordinates:  # Rechte Körperhälfte
                        ...
                        if angR > 140:
                            print(f"Oberkörper weiter nach hinten/aufrichten {angR}")
                        if angR < 80:
                            print(f"Oberkörper weiter nach vorne/aufrichten {angR}")
                    ...
                    if 11 in idx_to_coordinates and 13 in idx_to_coordinates and 15 in idx_to_coordinates:  # Linke Körperhälfte
                        ...
                        if angL > 140:
                            print(f"Oberkörper weiter nach hinten/aufrichten {angL}")
                        if angL < 80:
                            print(f"Oberkörper weiter nach vorne/aufrichten {angL}")
                ...
                try:
                    ...
                    if 14 in idx_to_coordinates and 16 in idx_to_coordinates and 20 in idx_to_coordinates:  # Rechte Körperhälfte
                        ...
                        # Linie Rechts Ellenbogen/Handgelenk
                        cv2.line(image, (idx_to_coordinates[16]), (idx_to_coordinates[20]), thickness=4,
                                color=(255, 0, 255))
                        l2 = np.linspace(idx_to_coordinates[16], idx_to_coordinates[20], 100)

                        # Winkel Rechts Schulter/Ellenbogen/Handgelenk
                        angR = ang((idx_to_coordinates[14], idx_to_coordinates[16]),
                                (idx_to_coordinates[16], idx_to_coordinates[20]))
                    ...
                    if 13 in idx_to_coordinates and 15 in idx_to_coordinates:  # Linke Körperhälfte
                        ...
                        cv2.line(image, (idx_to_coordinates[15]), (idx_to_coordinates[19]), thickness=4,
                                color=(255, 0, 255))
                        l2 = np.linspace(idx_to_coordinates[15], idx_to_coordinates[19], 100)

                        # Winkel Links Schulter/Ellenbogen/Handgelenk
                        angL = ang((idx_to_coordinates[13], idx_to_coordinates[15]),
                                (idx_to_coordinates[15], idx_to_coordinates[19]))
